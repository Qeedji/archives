/**
 * Function: configuration-by-script for Qeedji AQS devices
 * Version: 1.10.23
...
|---------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------
| getSoftwareVersion()								| returns the OS versione of the AQS device, i.e.: "9.10.12"  
| getModel() 										| returns the model of the AQS device, i.e.: "TAB10", "AMP300", "ROOMTAB"
...
//setHostname("myHostname"); // set and force the hostname of the device on the IP network, visible for example through the UPnP discovery or DHCP server (hostname example: "TAB10s_H1", "AMP300_H2", "ROOMTAB_H3")
...
//let lanParsedExclList=["www.domain1.contoso.fr", "*.domain2.contoso.fr"]; // If you have Proxy exclusion list
...
// Allows to enable the Https tasks manager: uncomment the line after 
//enableHttpsTasksManager();
// Allows to disable the Https tasks manager: uncomment the line after 
//disableHttpsTasksManager();
// Allows to set the HTTP server URL: uncomment the line after  
//setHttpsTasksManagerUrl("http://<myHttpServerDomain>/<directory_to_get_the_task_json_content>"); // ex1: https://demo.http.server/administration?id=task, ex2: https://demo.http.server/administration/task.json
// Allows to set credentials to basic authentication: uncomment the two lines after 
//setHttpsTasksManagerCredential("user-password"); // user-password is mandatory
//setHttpsTasksManagerCreateCredentialForBasicAuthentication("user-password", "myHttpServerIdentifier", "myHttpServerPassword");
// Allows to set credentials to none: uncomment the line after 
//setHttpsTasksManagerCreateCredentialNone();
// Allows to set the hearbeat: uncomment the line after 
//setHttpsTasksManagerHeartbeat(10); // default: 10 min
...
if (platform === "tab10") {
	...
	// ---- Allows to activate/inactivate the Rfid125KHz sensor for TAB10/ROOMTAB device: uncomment one of the following lines.
...
function enableHttpsTasksManager()
{
    Android.Preferences("SharedPreferences", "tech.qeedji.system.app", "tech.qeedji.system.app_preferences").setBoolean("httpstasksmanager.enabled", true);
}
function disableHttpsTasksManager()
{
    Android.Preferences("SharedPreferences", "tech.qeedji.system.app", "tech.qeedji.system.app_preferences").setBoolean("httpstasksmanager.enabled", false);
}
function setHttpsTasksManagerUrl (aUrl)
{
    Android.Preferences("SharedPreferences", "tech.qeedji.system.app", "tech.qeedji.system.app_preferences").setString("httpstasksmanager.url", aUrl);
}

function setHttpsTasksManagerCreateCredentialForBasicAuthentication(aCredential, aUsername, aPassword) {
    var basename = "tech.qeedji.system.app_preferences.httpstasksmanager.credential." + aCredential + ".prefs";
    Android.Preferences("SharedPreferences", "tech.qeedji.system.app", basename).setString("type", "user-password"); 
    Android.Preferences("SharedPreferences", "tech.qeedji.system.app", basename).setString("username", aUsername);
    Android.Preferences("SharedPreferences", "tech.qeedji.system.app", basename).setString("password", aPassword);
}
function setHttpsTasksManagerCreateCredentialNone() {
     Android.Preferences("SharedPreferences", "tech.qeedji.system.app", "tech.qeedji.system.app_preferences").setString("httpstasksmanager.credential", "");
}
function setHttpsTasksManagerCredential(aCredential)
{
    Android.Preferences("SharedPreferences", "tech.qeedji.system.app", "tech.qeedji.system.app_preferences").setString("httpstasksmanager.credential", aCredential);
}	
function setHttpsTasksManagerHeartbeat(aHeartbeat)
{
    Android.Preferences("SharedPreferences", "tech.qeedji.system.app", "tech.qeedji.system.app_preferences").setInt("httpstasksmanager.heartbeat", aHeartbeat);
}							  